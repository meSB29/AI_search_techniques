...
# Expands the given node and stores its all valid child nodes.
	...
	if   marker_pos[0]-1>=0  :
		...
		if hash(copy.deepcopy(node)) not in visited:
			visited[hash(copy.deepcopy(node))]=False	
		node=swapPositions(node,marker_pos[0]-1,marker_pos[1],marker_pos[0],marker_pos[1])
...
#			h : the heuristic function . May have values h1 or h2.
	...
	while(pq.empty()==False):
		...
		visited[hash(copy.deepcopy(min_node))]=True
		if(min_node==goal):
			print("Success!!")
			print("Start State : ")
			for i in source:
